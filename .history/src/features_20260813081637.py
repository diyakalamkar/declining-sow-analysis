...
def build_customer_features(txn, cust, sow):
    ...
    # Silent attrition flag: had ABC-card activity in an early window but zero in the most recent window
    abc_txn = txn[txn["Payment_Code"] == 3]
    cutoff_recent = as_of - pd.DateOffset(months=3)
    cutoff_early_start = as_of - pd.DateOffset(months=6)

    had_early_abc = abc_txn[(abc_txn["Transaction_Date"] >= cutoff_early_start) &
                             (abc_txn["Transaction_Date"] < cutoff_recent)]["Customer_ID"].unique()
    has_recent_abc = abc_txn[abc_txn["Transaction_Date"] >= cutoff_recent]["Customer_ID"].unique()

    silent_attrition_ids = set(had_early_abc) - set(has_recent_abc)
    feats["is_silent_attrition"] = feats["Customer_ID"].isin(silent_attrition_ids).astype(int)
    
    feats[num_cols] = feats[num_cols].fillna(0)

    # for headline slide
    n_silent = feats["is_silent_attrition"].sum()
    recoverable = feats.loc[feats["is_silent_attrition"]==1, "total_lifetime_spend"].sum()
    print(f"Silent attrition cohort: {n_silent} customers ({n_silent/len(feats):.1%} of base)")
    print(f"Their lifetime XYZ spend (now going elsewhere): ₹{recoverable:,.0f}")
