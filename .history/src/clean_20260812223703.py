...
def clean_dates(df, cols):
    for c in cols:
        s = df[c]
        # try the two most likely formats explicitly, in order, without a dayfirst guess
        parsed = pd.to_datetime(s, format="%d-%m-%Y", errors="coerce")
        remaining = parsed.isna() & s.notna()
        if remaining.any():
            parsed2 = pd.to_datetime(s[remaining], format="%Y-%m-%d", errors="coerce")
            parsed.loc[remaining] = parsed2
        # last-resort generic parse for anything still unparsed
        still_bad = parsed.isna() & s.notna()
        if still_bad.any():
            parsed.loc[still_bad] = pd.to_datetime(s[still_bad], errors="coerce", dayfirst=True)
        df[c] = parsed
    return df
