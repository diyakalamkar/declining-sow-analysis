...
def build_customer_features(txn, cust, sow):
    ...
    # overall + recent (last 3 fiscal months) SoW
    ...
    feats["sow_trend"] = feats["recent_SoW"] - feats["early_SoW"]   # negative = declining

    # RFM + category mix (all spend at XYZ, regardless of payment method)
    ...
    cat_mix = (txn.groupby(["Customer_ID", "Category"])["Net_Amount"].sum()
               .unstack(fill_value=0))
    cat_mix = cat_mix.div(cat_mix.sum(axis=1).replace(0, np.nan), axis=0).add_prefix("pct_")

    pay_mix = (txn.groupby(["Customer_ID", "Method"])["Net_Amount"].sum()
               .unstack(fill_value=0))
    ...
    feats = (feats
             ...
             .merge(cust[["Customer_ID","Age","Gender","Members","Credit_Card_Open_Date",
                           "Credit_Card_Close_Date","Credit_Card_Limit","Credit_Card_APR",
