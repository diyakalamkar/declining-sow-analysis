...
def run(feats: pd.DataFrame, n_clusters: int = 4) -> pd.DataFrame:
    ...
    # winsorize every clustering column at 1st/99th percentile to stop single outliers hijacking a cluster
    for c in X_raw.columns:
        lo, hi = X_raw[c].quantile([0.01, 0.99])
        X_raw[c] = X_raw[c].clip(lo, hi)

    # RobustScaler (median/IQR) is less outlier-sensitive than StandardScaler (mean/std)
    ...
    # Override: silent attrition takes priority over whatever cluster they landed in
    # silent_mask = feats["is_silent_attrition"] == 1
    # feats.loc[silent_mask, "micro_segment_name"] = "Silent Attrition (Card Active, Unused)"
    silent_mask = feats["is_silent_attrition"] == 1
    feats["micro_segment_name"] = feats["micro_segment"].apply(
        lambda x: f"Segment {int(x)}" if pd.notna(x) else "Not Targeted (High/Dormant SoW)"
    )
    feats.loc[silent_mask, "micro_segment_name"] = "Silent Attrition (Card Active, Unused)"

    from src import segmentation
    feats = segmentation.run(feats, n_clusters=4)  # match whatever n_clusters you actually use
    target = feats[feats["sow_tier"].isin(["Low SoW - At Risk", "Moderate SoW - Growable"])]
    print(target.groupby("micro_segment")[segmentation.CLUSTER_COLS].mean().round(3))
    print(target.groupby("micro_segment").size())
