...
with tab1:
    ...
    # keep as real datetime (not string) so Plotly's vline math works
    trend["year_month"] = pd.to_datetime(trend["year_month"].astype(str))
    ...
    cliff_date = pd.Timestamp("2025-08-01")
    if (trend["year_month"] == cliff_date).any():
        fig.add_vline(x=cliff_date.timestamp() * 1000, line_dash="dash", line_color="#E5484D",
    ...
    fig.update_layout(yaxis_tickformat=".0%", plot_bgcolor="white", height=450,
                      xaxis_title="Month", yaxis_title="Portfolio SoW")
...
with tab3:
    ...
    top_df = feats.sort_values("priority_score", ascending=False).head(top_n)[display_cols].copy()

    # pre-format as display strings instead of using .style (avoids jinja2 dependency)
    if "sow_risk_score" in top_df.columns:
        top_df["sow_risk_score"] = top_df["sow_risk_score"].map("{:.2f}".format)
    if "avg_SoW" in top_df.columns:
        top_df["avg_SoW"] = top_df["avg_SoW"].map("{:.1%}".format)
    if "recoverable_spend" in top_df.columns:
        top_df["recoverable_spend"] = top_df["recoverable_spend"].map("₹{:,.0f}".format)
    if "priority_score" in top_df.columns:
        top_df["priority_score"] = top_df["priority_score"].map("{:,.0f}".format)
