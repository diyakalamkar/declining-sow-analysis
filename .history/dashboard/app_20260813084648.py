...
with tab1:
    ...
    # year_month may arrive as Period OR Timestamp depending on parquet round-trip — normalize explicitly
    trend["year_month"] = pd.to_datetime(trend["year_month"].astype(str)).dt.strftime("%Y-%m")
    ...
    if "2025-08" in trend["year_month"].values:
